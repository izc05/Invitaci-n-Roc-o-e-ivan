(()=>{
  ...
  function updateCountdown(){
    ...
    $('#v106Days') && ($('#v106Days').textContent=d);
    $('#v106Hours') && ($('#v106Hours').textContent=String(h).padStart(2,'0'));
    $('#v106Minutes') && ($('#v106Minutes').textContent=String(m).padStart(2,'0'));
    $('#v106Seconds') && ($('#v106Seconds').textContent=String(s).padStart(2,'0'));
  }
    ...
    eventSection.insertAdjacentHTML('afterend',`
      <section class="v106-place" id="v106Place">
        <div class="v106-place__sticky">
          <div class="v106-place__photo" role="img" aria-label="Jardines de Finca Los Olivos, El Vellón"></div>
  ...
  function parallaxV106(){
    ...
    if(place&&!reduce){const p=sceneProgress(place),photo=place.querySelector('.v106-place__photo');if(photo)photo.style.transform=`translate3d(0,${-85+170*p}px,0) scale(${1.15-.07*p})`;}
    ticking=false;
  }
  addEventListener('scroll',()=>{if(!ticking){ticking=true;requestAnimationFrame(parallaxV106)}},{passive:true});
  addEventListener('resize',parallaxV106,{passive:true}); parallaxV106();
})();
